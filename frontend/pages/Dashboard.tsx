        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-[var(--bg-secondary)] p-5 rounded-2xl border border-[var(--border-primary)] shadow-sm hover:shadow-md transition-all group">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-lg ${stat.color === 'emerald' ? 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-200 border border-transparent dark:border-emerald-500/20' :
                  stat.color === 'orange' ? 'bg-orange-50 dark:bg-amber-500/15 text-orange-600 dark:text-amber-200 border border-transparent dark:border-amber-500/20' :
                    stat.color === 'blue' ? 'bg-blue-50 dark:bg-sky-500/15 text-blue-600 dark:text-sky-200 border border-transparent dark:border-sky-500/20' :
                      'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-200 border border-transparent dark:border-emerald-500/20'
                }`}>
                {stat.trend}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 bg-[var(--bg-secondary)] rounded-[32px] border border-[var(--border-primary)] shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-[var(--border-primary)] flex justify-between items-center bg-[var(--bg-accent)]">
            ...
            <span className="px-3 py-1 bg-emerald-100 dark:bg-white/10 text-emerald-700 dark:text-white/80 border border-transparent dark:border-white/10 rounded-full text-xs font-black uppercase">
                    ...
                    <tr key={pausa.id} className="hover:bg-[var(--bg-accent)] transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`size-10 rounded-xl bg-slate-200 dark:bg-gray-800 flex items-center justify-center text-slate-900 dark:text-[var(--text-primary)] font-black`}>
            ...
            {activeBreaks.length === 0 ? (
              ...
              activeBreaks.slice(0, 5).map((activity) => (
                <div key={activity.id} className="bg-[var(--bg-secondary)] p-4 rounded-xl border border-[var(--border-primary)] shadow-sm flex items-center gap-3 group hover:border-emerald-500/30 transition-all">
                  <div className={`size-10 rounded-lg bg-emerald-100 dark:bg-emerald-500/15 flex items-center justify-center text-emerald-800 dark:text-emerald-200 font-bold border border-emerald-200 dark:border-emerald-500/20`}>
